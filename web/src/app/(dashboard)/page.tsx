  ...
  return (
    <>
      ...
      <section className="animate-rise-in overflow-hidden rounded-3xl bg-[#17211b] p-6 text-white md:p-8">
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="inline-flex items-center gap-2 rounded-full bg-white/8 px-3 py-1 text-xs font-medium text-[#d8ff62]"><Sparkles className="size-3.5" />Live office pulse</p><h2 className="mt-4 text-3xl font-semibold tracking-tight">{open.length ? `${open.length} ${open.length === 1 ? "person is" : "people are"} building today.` : "Ready for the next check-in."}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-white/55">Attendance is live from your connected terminals. Raw activity and rounded reporting stay separately auditable.</p></div><a href="/events" className="inline-flex items-center gap-2 text-sm font-medium text-[#d8ff62] transition hover:translate-x-1">Open terminal event feed <ArrowUpRight className="size-4" /></a></div>
      </section>
