...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <>
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
      <div className="animate-rise-in rounded-2xl border border-black/6 bg-white p-5 shadow-sm shadow-black/[.02]"><div className="flex items-start justify-between"><div><h2 className="font-semibold">Daily attendance</h2><p className="mt-1 text-sm text-black/45">Rounded team hours by day. Hover a bar for detail.</p></div><BarChart3 className="size-5 text-black/35" /></div><div className="mt-8 flex h-52 items-end gap-1.5">{activity.map(({ day, minutes }, index) => <div key={day.toISOString()} className="group flex h-full min-w-0 flex-1 items-end"><div title={`${format(day, "EEE d MMM")}: ${formatDuration(minutes)}`} className="w-full origin-bottom rounded-t-md bg-[#d8ff62] transition-all duration-700 ease-out group-hover:bg-[#b9df45]" style={{ height: `${Math.max(minutes ? 4 : 1, minutes / maxDaily * 100)}%`, transitionDelay: `${index * 18}ms` }} /></div>)}</div><div className="mt-3 flex justify-between text-[10px] text-black/40"><span>{format(start, "d MMM")}</span><span>{format(end, "d MMM")}</span></div></div>
