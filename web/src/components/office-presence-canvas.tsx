...
type Node = Person & { x: number; y: number; homeX: number; homeY: number; vx: number; vy: number; r: number };
...
export function OfficePresenceCanvas({ people }: { people: Person[] }) {
  ...
  useEffect(() => {
    ...
    if (!context) return;
    let frame = 0, width = 1, height = 1, dpr = 1, previous = performance.now();
    const nodes: Node[] = people.map((person, index) => { const x = 55 + (index % 3) * 72, y = 54 + Math.floor(index / 3) * 70; return { ...person, x, y, homeX: x, homeY: y, vx: (index % 2 ? .12 : -.12), vy: .06 + (index % 3) * .03, r: 25 }; });
    ...
    const render = (time: number) => {
      const step = Math.min(2, (time - previous) / 16.67); previous = time;
      ...
      for (let a = 0; a < nodes.length; a++) for (let b = a + 1; b < nodes.length; b++) {
        const dx = nodes[b].x - nodes[a].x, dy = nodes[b].y - nodes[a].y, distance = Math.max(1, Math.hypot(dx, dy)), minimum = nodes[a].r + nodes[b].r + 10;
        if (distance < minimum) { const force = (minimum - distance) * .0025; const x = dx / distance * force, y = dy / distance * force; nodes[a].vx -= x; nodes[a].vy -= y; nodes[b].vx += x; nodes[b].vy += y; }
      }
      for (const node of nodes) {
        // A weak spring keeps the arrangement calm; velocity is capped so a
        // drag settles naturally rather than producing a noisy bounce.
        node.vx += (node.homeX - node.x) * .00045 * step; node.vy += (node.homeY - node.y) * .00045 * step;
        node.vx *= .988; node.vy *= .988;
        const speed = Math.hypot(node.vx, node.vy), maxSpeed = .62;
        if (speed > maxSpeed) { node.vx = node.vx / speed * maxSpeed; node.vy = node.vy / speed * maxSpeed; }
        node.x += node.vx * step; node.y += node.vy * step;
        if (node.x < node.r || node.x > width - node.r) { node.x = Math.max(node.r, Math.min(width - node.r, node.x)); node.vx *= -.45; }
        if (node.y < node.r || node.y > height - node.r) { node.y = Math.max(node.r, Math.min(height - node.r, node.y)); node.vy *= -.45; }
    ...
    const observer = new ResizeObserver(resize); observer.observe(canvas); resize(); frame = requestAnimationFrame(render); canvas.addEventListener("pointerdown", pointerDown); canvas.addEventListener("pointermove", pointerMove); canvas.addEventListener("pointerup", pointerUp); canvas.addEventListener("pointercancel", pointerUp);
